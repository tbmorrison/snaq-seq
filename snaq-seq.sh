...
NO_ARGS=0
if [ $# -eq "$NO_ARGS" ]; then
	echo -e "***\n [ERROR] No options were provided. Please run 'snaq-seq.sh -h' for more information. *** \n"
    	exit 1
fi
...
if [[ "$1" == "-b" ]]; then
	if [[ -f "$2" ]]; then
	       ...
	       output=true
               ...
               docker run -e bc_prefix="$bc_prefix" -e bc_output="$bc_output" -v $2:/home/basechange/reference_amplicon.fasta  -v $bc_output:/home/basechange/ -ti accugenomics/snaq-seq:pre bash /preliminary/init_bc.sh
	       fi
	       exit
	fi	
else
...
#if [[ "$1" == "-i" ]]; then
#        reference_genome=$(echo ${2} | sed 's@.*/@@' )
#        indices_output=$(echo ${2} | sed 's/\/[^/]*$/\//')
#        docker run -e reference_genome="$reference_genome" -e indices_output="$indices_output"  -v $2:/home/genome/reference_genome.fasta -ti accugenomics/snaq-seq:pre bash /preliminary/init_bwa.sh
#fi
...
if [[ ! "$1" == "-b" ]]; then
	if [[ ! -d "$arg_2_option2" ]]; then
	        output=false
	        echo -e "***\n [ERROR] - The 'output=' option is provided with no existing directory. Please run 'snaq-seq.sh -h' for more information. *** \n"
	        exit 1
	fi
else
