...
#
## Usage: perl /snaq-seq/snaq-seq_core.pl $inputFILE_fasta $genome_fasta $outsam $ofs $mfs $rc $mpq $qc $gbc $outis $cc $is 
#
...
my $launch_1 = "mkdir /home/output/$time; cd /home/ ; bash /snaq-seq/make_dir.sh; bash /snaq-seq/snaq-seq_command.sh; bash /snaq-seq/snaq-seq_launch.sh; sed -i \'s/TIME/$time/g\'  make_dir_list.sh; sed -i \'s/TIME/$time/g\'  snaq-seq_launch_list.sh; sed -i \'s/TIME/$time/g\' snaq-seq_runs_list.sh;  sed -i \'s/GENOME/$genome_fasta/g\' snaq-seq_runs_list.sh;  sed -i \'s/REPLACE/-v $offspringCutoff -v $mfs -v $RC -v $mapq -v $qCutoff -v $gbc -v $outputIS/g\' snaq-seq_runs_list.sh; sed -i \'s/TIME/$time/g\' output_1.sh; sed -i \'s/TIME/$time/g\' output_2.sh; bash snaq-seq_runs_list.sh"; 
system($launch_1);
wait;
print "******************************************************\n";
print "*                                                    *\n";
...
my $launch_5 = "Rscript /snaq-seq/CombineThreads.R '/home/output/$time' '/home/data/output' '/home/data/normalization' $offspringCutoff >> /home/error2" ; 
system($launch_5);
